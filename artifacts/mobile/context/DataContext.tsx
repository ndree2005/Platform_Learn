...
interface DataContextType {
  ...
  users: AppUser[];
  loading: boolean;
  addCourse: (course: Omit<Course, "id" | "createdAt" | "enrolledStudents" | "rating">) => Promise<void>;
  updateCourse: (id: string, updates: Partial<Course>) => Promise<void>;
  deleteCourse: (id: string) => Promise<void>;
  enrollInCourse: (userId: string, courseId: string) => Promise<void>;
  addAssignment: (assignment: Omit<Assignment, "id">) => Promise<void>;
  deleteAssignment: (id: string) => Promise<void>;
  submitAssignment: (assignmentId: string, studentId: string, studentName: string, answer: string) => Promise<void>;
  ...
  addUser: (user: Omit<AppUser, "id" | "joinDate">) => Promise<void>;
...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const addCourse = async (course: Omit<Course, "id" | "createdAt" | "enrolledStudents" | "rating">) => {
    const newCourse = {
      ...course,
      id: genId(),
  ...
  const enrollInCourse = async (userId: string, courseId: string) => {
    const updated = await api.post<Course>(`/courses/${courseId}/enroll`, { studentId: userId });
  ...
  const submitAssignment = async (
    ...
    const created = await api.post<Submission>(`/assignments/${assignmentId}/submit`, payload);
    setSubmissions((prev) => [...prev, created]);
  };

  const gradeSubmission = async (submissionId: string, score: number) => {
    const updated = await api.put<Submission>(`/submissions/${submissionId}/grade`, { score });
    setSubmissions((prev) => prev.map((s) => (s.id === submissionId ? updated : s)));
  };

  // ── Progress ──────────────────────────────────────────────────────────────

  const updateProgress = async (userId: string, courseId: string, lessonId: string) => {
    const course = courses.find((c) => c.id === courseId);
    const existing = progress.find((p) => p.userId === userId && p.courseId === courseId);
    ...
    setProgress((prev) => {
      const filtered = prev.filter((p) => !(p.userId === userId && p.courseId === courseId));
  ...
  const addUser = async (user: Omit<AppUser, "id" | "joinDate">) => {
    const payload = {
      ...user,
      id: genId(),
  ...
  return (
    <DataContext.Provider
      value={{
        courses, assignments, submissions, progress, users, loading,
