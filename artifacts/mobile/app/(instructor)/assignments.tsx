...
export default function InstructorAssignments() {
  ...
  const handleGrade = () => {
    ...
    if (isNaN(score) || score < 0 || score > gradeModal.max) {
      Alert.alert(
        "Invalid Score",
        `Score must be between 0 and ${gradeModal.max}`,
      );
      return;
    }
              ...
              >
                <View style={styles.cardHeader}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text
                      style={[styles.cardCourse, { color: colors.primary }]}
                    >
                      {a.courseName}
                    </Text>
                  </View>
                ...
                {subs.length > 0 && (
                  <View style={styles.subsSection}>
                    <Text
                      style={[styles.subsTitle, { color: colors.foreground }]}
                    >
                      Submissions: {graded.length} graded, {pending.length}{" "}
                      pending
                    </Text>
                    {subs.map((sub) => (
                      <View
                        key={sub.id}
                        style={[styles.subRow, { borderColor: colors.border }]}
                      >
                        <View style={{ flex: 1 }}>
                          <Text
                            style={[
          ...
          <View style={[styles.sheet, { backgroundColor: colors.card }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <CourseCombobox
                ...
                onQueryChange={setCourseSearchQuery}
                onSelect={(course) => {
        ...
        <View style={styles.overlay}>
          <View style={[styles.gradeSheet, { backgroundColor: colors.card }]}>
            ...
            <View style={styles.gradeActions}>
              <TouchableOpacity
                style={[styles.gradeCancelBtn, { borderColor: colors.border }]}
                onPress={() => setGradeModal(null)}
              >
                <Text
                  style={[
                    styles.gradeCancelText,
                    { color: colors.mutedForeground },
                  ]}
                >
                  Cancel
