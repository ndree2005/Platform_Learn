...
export default function AdminUsers() {
  ...
  const handleAdd = () => {
    ...
    addUser({ name: newName.trim(), email: newEmail.trim(), role: newRole, isActive: true });
  ...
  const handleDelete = (u: AppUser) => {
    Alert.alert("Delete User", `Remove "${u.name}" from the system?`, [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: () => { deleteUser(u.id); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); },
      },
    ]);
  };
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll} contentContainerStyle={styles.filterContent}>
        {ROLES.map((r) => (
          <TouchableOpacity key={r}
            style={[styles.chip, { borderColor: colors.border, backgroundColor: roleFilter === r ? colors.primary : colors.card }]}
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 90 }]} showsVerticalScrollIndicator={false}>
        {filtered.length === 0 ? (
          ...
          filtered.map((u) => <UserCard key={u.id} user={u} onDelete={() => handleDelete(u)} />)
        )}
      </ScrollView>
        ...
        <View style={styles.overlay}>
          <View style={[styles.sheet, { backgroundColor: colors.card }]}>
            ...
            <View style={styles.sheetHeader}>
              <Text style={[styles.sheetTitle, { color: colors.foreground }]}>Add User</Text>
            ...
            <View style={styles.field}>
              ...
              <TextInput style={[styles.input, { backgroundColor: colors.muted, borderColor: colors.border, color: colors.foreground }]}
                value={newName} onChangeText={setNewName} placeholder="Full name" placeholderTextColor={colors.mutedForeground} />
            </View>
            <View style={styles.field}>
              <Text style={[styles.fieldLabel, { color: colors.mutedForeground }]}>Email *</Text>
              <TextInput style={[styles.input, { backgroundColor: colors.muted, borderColor: colors.border, color: colors.foreground }]}
                value={newEmail} onChangeText={setNewEmail} placeholder="user@email.com" placeholderTextColor={colors.mutedForeground}
            ...
            <View style={styles.field}>
              ...
              <View style={styles.roleRow}>
                {(["student", "instructor", "admin"] as UserRole[]).map((r) => {
                  const roleColors: Record<UserRole, string> = { student: "#0CA678", instructor: "#3B5BDB", admin: "#9B59B6" };
                  return (
                    <TouchableOpacity key={r}
                      style={[styles.roleChip, { borderColor: newRole === r ? roleColors[r] : colors.border, backgroundColor: newRole === r ? roleColors[r] + "18" : colors.muted }]}
                      onPress={() => setNewRole(r)}>
                      <Text style={{ fontSize: 13, fontWeight: "600", color: newRole === r ? roleColors[r] : colors.mutedForeground, textTransform: "capitalize" }}>{r}</Text>
                    </TouchableOpacity>
                  );
                })}
              </View>
...
const styles = StyleSheet.create({
  ...
  sheet: { borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, gap: 12, paddingBottom: 32 },
  ...
  roleChip: { flex: 1, paddingVertical: 10, borderRadius: 10, borderWidth: 1, alignItems: "center" },
